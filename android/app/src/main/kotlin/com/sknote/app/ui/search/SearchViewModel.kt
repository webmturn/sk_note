...
class SearchViewModel : ViewModel() {
    ...
    fun search(query: String) {
        viewModelScope.launch {
            ...
            _error.value = null
            try {
                val response = ApiClient.getService().getArticles(search = query, limit = 50)
                if (response.isSuccessful) {
                    _results.value = response.body()?.articles ?: emptyList()
                } else {
                    _error.value = "搜索失败"
                }
            } catch (e: Exception) {
    ...
    fun loadAll() {
        viewModelScope.launch {
            ...
            _error.value = null
            try {
                val response = ApiClient.getService().getArticles(limit = 50)
                if (response.isSuccessful) {
                    _results.value = response.body()?.articles ?: emptyList()
                } else {
                    _error.value = "加载失败"
